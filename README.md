...
- **GitHub Push 상시 수행**: 모든 유의미한 코드 수정, UI 개편, 버그 수정 완료 후에는 반드시 `git push` 실행 (App 헤더 푸시 시간 자동화됨).
- **💡 AI 스마트 채우기 팁 (자주 하는 실수)**: 네이버 지도 **링크만** 복사해서 붙여넣으면 AI(Groq)는 내용을 읽을 수 없어 실패합니다. 반드시 네이버 지도 상세 페이지에서 **텍스트를 길게 눌러 [전체 선택] → [복사]** 한 뒤 'AI 스마트 채우기'를 사용하세요. (링크 포함 30자 이상의 텍스트 권장)
