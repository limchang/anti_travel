...
export default async function handler(req, res) {
  ...
  const apiKey = requestApiKey || await getStoredPerplexityApiKey(req) || process.env.PERPLEXITY_API_KEY;
  if (!apiKey) {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        error: 'Perplexity nearby recommend failed',
    ...
    return res.status(500).json({
      error: 'Unexpected Perplexity recommend error',
