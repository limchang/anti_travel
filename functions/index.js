...
const fetchKakaoJson = async (url, restKey) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`kakao http ${response.status}: ${body.slice(0, 180)}`);
  }
...
const tryKeywordSearch = async (query, restKey) => {
  ...
  if (Array.isArray(kwJson?.documents) && kwJson.documents.length > 0) {
    ...
    if (lat != null && lon != null) return { lat, lon, source: 'keyword', query, placeName: String(doc.place_name || '').trim() };
  }
  return null;
};
...
const tryAddressSearch = async (query, restKey) => {
  ...
  if (Array.isArray(addrJson?.documents) && addrJson.documents.length > 0) {
    ...
    const lat = toNum(doc.y);
    const lon = toNum(doc.x);
    if (lat != null && lon != null) return { lat, lon, source: 'address', query };
  }
  return null;
};
...
const geocodeWithKakao = async ({ address, placeName = '', restKey }) => {
  ...
  const nameQueries = queries.filter((query) => trimmedName && query.includes(trimmedName));
...
exports.routeVerify = onRequest({ invoker: 'public' }, async (req, res) => {
  setCorsPublic(req, res);
  if (req.method === 'OPTIONS') return res.status(204).send('');
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  ...
  try {
    ...
    if (!candidates.length) {
      return res.status(502).json({ error: 'kakao directions failed' });
    }
