...
import MapOverlayBar from './components/map/MapOverlayBar.jsx';
import OverviewWidget from './components/shared/OverviewWidget.jsx';
import TopMenuBar from './components/shared/TopMenuBar.jsx';
...
import {
  ...
  ChevronsRight, Sparkles, Wand2, CornerDownRight, Store, Lock, Unlock, ChevronLeft, ChevronRight, Timer, Anchor, Utensils, Coffee, Camera, Bed, MoonStar, ChevronDown, ChevronUp, Package, Eye, Star, Pencil, Calendar, CalendarDays, Gift, X, Share2, SlidersHorizontal, LoaderCircle, Info, RotateCcw, Zap, Home, Clock, Soup, GripVertical
} from 'lucide-react';
...
const App = () => {
  ...
  const getCategoryIcon = (type) => {
    ...
    const ic = 12;
    switch (type) {
      case 'food': return <div key={type} className={`${style} text-rose-500 bg-red-50 border-red-100`}><Utensils size={ic} /></div>;
      case 'cafe': return <div key={type} className={`${style} text-amber-900 bg-amber-50 border-amber-200`}><Coffee size={ic} /></div>;
      case 'tour': return <div key={type} className={`${style} text-purple-600 bg-purple-50 border-purple-100`}><Camera size={ic} /></div>;
      case 'lodge': return <div key={type} className={`${style} text-indigo-600 bg-indigo-50 border-indigo-100`}><Bed size={ic} /></div>;
  ...
  if (!user && !sharedSource?.ownerId) {
    return <LoginScreen handleLogin={handleLogin} handleGuestMode={handleGuestMode} authError={authError} />;
  }
  ...
  return (
    <div className="min-h-screen bg-[#F2F4F6] text-[#191F28] font-sans flex overflow-x-hidden font-bold flex-col relative">
      <TopMenuBar
        ...
        isCalculatingAllRoutes={isCalculatingAllRoutes} routeCalcProgress={routeCalcProgress}
        setShowShareManager={setShowShareManager}
      />
      {/* ── 장소 수정 모달 ── */}
      ...
      >
        ...
        {(mapEditMode || isMobileLayout) && (
          <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-100 shrink-0">
            ...
            {/* 구 고정 헤더 — 완전 삭제 (mapEditMode 항상 true) */}
            {/* ── 스크롤 컨텐츠 ── */}
            <div
                  ...
                  >
                    ...
                    <div id="right-panel-map-overview" className={(mapEditMode && !isMobileLayout) || isMobileLayout
                      ...
                      <div className="relative overflow-visible transition-colors duration-300 w-full h-full">
                        ...
                        />
                        <MapOverlayBar
                          mapDayOptions={mapDayOptions}
                          routePreviewEndpointActions={routePreviewEndpointActions}
                            ...
                            try {
                              ...
                              if (parsed) {
                                setItinerary(prev => {
                                  ...
                                  if (parsed.business && Object.keys(parsed.business).length) target.business = normalizeBusiness({ ...(target.business || {}), ...parsed.business });
                                  if (parsed.menus?.length) { if (!target.receipt) target.receipt = {}; target.receipt.items = buildSmartFillMenuItems(parsed.menus); }
                                  if (parsed.name && !target.name) target.name = parsed.name;
                                  return next;
                                });
                                showInfoToast(isAiSmartFillSource(result?.source) ? `AI가 ${place.name} 정보를 분석해 입력했습니다.` : `${place.name} 정보를 스마트 입력했습니다.`);
                              } else {
                                showInfoToast(useAiSmartFill ? 'AI가 정보를 찾지 못했습니다.' : '정보를 찾지 못했습니다.');
                              }
                            } catch (err) {
                              showInfoToast(getSmartFillErrorMessage(err, useAiSmartFill));
                            }
                          }}
        ...
        >
          ...
          {showTimelineOverlay && (
            <div className="sticky top-0 z-10 flex items-center justify-center py-2">
              <button type="button" onClick={() => setShowTimelineOverlay(false)} className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-[11px] font-black text-slate-500 transition-colors">
                <ChevronDown size={12} /> 지도로 돌아가기
              </button>
            </div>
          )}
          {isSharedReadOnly && (
          ...
          {showPlanManager && <PlanManagerModal planList={planList} currentPlanId={currentPlanId} setCurrentPlanId={setCurrentPlanId} createNewPlan={createNewPlan} resolvePlanMetaForCard={resolvePlanMetaForCard} getRegionCoverImage={getRegionCoverImage} setLastAction={setLastAction} />}
          {showPlaceTrash && <PlaceTrashModal itinerary={itinerary} restorePlaceFromTrash={restorePlaceFromTrash} deletePlacePermanently={deletePlacePermanently} />}
          ...
          {showPlanOptions && <PlanOptionsModal planOptionRegion={planOptionRegion} setPlanOptionRegion={setPlanOptionRegion} planOptionStartDate={planOptionStartDate} setPlanOptionStartDate={setPlanOptionStartDate} planOptionEndDate={planOptionEndDate} setPlanOptionEndDate={setPlanOptionEndDate} planOptionBudget={planOptionBudget} setPlanOptionBudget={setPlanOptionBudget} collaborators={collaborators} setCollaborators={setCollaborators} manualSaveHistory={manualSaveHistory} canManagePlan={canManagePlan} collaboratorLoading={collaboratorLoading} setCollaboratorLoading={setCollaboratorLoading} saveItineraryManually={saveItineraryManually} restoreSaveHistory={restoreSaveHistory} setTripRegion={setTripRegion} setTripStartDate={setTripStartDate} setTripEndDate={setTripEndDate} setItinerary={setItinerary} setShowHeroSummaryModal={setShowHeroSummaryModal} />}
          ...
          <>
            ...
            >
              ...
              >
                ...
                {isPlaceQuickView && (
                  <button type="button" onClick={async (e) => { e.stopPropagation(); const result = await searchAddressFromPlaceName(qvItem.name || '', tripRegion); if (result?.address) { setItinerary(prev => { const next = structuredClone(prev); const target = (next.places || []).find(pl => pl.id === qvItem.id); if (target) { target.address = result.address; if (!target.receipt) target.receipt = {}; target.receipt.address = result.address; } return next; }); showInfoToast(`'${qvItem.name}' 주소 자동 채움`); } else { showInfoToast('주소를 찾지 못했습니다.'); } }} className="shrink-0 w-7 h-7 flex items-center justify-center rounded-lg bg-white/20 text-white hover:bg-white/30 transition-colors" title="주소 자동 채우기"><Sparkles size={12} /></button>
                )}
                {isPlaceQuickView && (
